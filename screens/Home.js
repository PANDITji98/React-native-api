          ...
          {data &&
            data.map((values, index) => {
              return (
                <View
                  key={index}
                  className={`flex justify-between items-center flex-col h-[380px] w-[380px] bg-[#f3f3f3] p-1.5 rounded-[10px] border-[1.5px] border-solid border-[black] mt-[40px] ${
                    data.length == index + 1 ? "mb-[60px]" : ""
                  }`}
                >
                  ...
                  <Image
                    source={{
                      uri: values.mediaId
                        ? values.mediaId
                        : `https://api.slingacademy.com/public/sample-photos/${
                            index + 1
                          }.jpeg`,
                    }}
